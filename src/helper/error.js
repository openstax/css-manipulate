...
// This selector runs in the browser (chrome) but
// needs to match the selector format generated by jsdom.
// TODO: Move this into a separate file so it can be used by headless Chrome and nodejs (for jsdom, so we can maintain sourcemaps)
function constructSelector(el) {
  if (!el) {
    ...
    const myIndex = nodesAry.filter((node) => node.nodeType === Node.ELEMENT_NODE /*1*/).indexOf(el)
