...
// This is for declarations that are not processed. like `font-weight: bold`
...
function VanillaDeclarationFactory(engine, property) {
  return new RuleDeclaration(property, ($, $lookupEl, $elPromise, vals, astNode) => {
    ...
    // convert the rule into an autogenerated class name
    const declarationValue = vals.map((v) => v.join(' ')).join(',') // TODO: handle !important.
    const autogenClassName = engine.addVanillaRule(property, declarationValue)
...
module.exports = {DECLARATIONS, VanillaDeclarationFactory}
